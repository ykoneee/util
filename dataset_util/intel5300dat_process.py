...
def get_csi_from_bytes(byte_data):
    ...
    cfr_array = np.zeros((len(csi_trace), 3, 30), dtype=np.complex128)
...
def test():
    ...
    with zipfile.ZipFile(
            # "/media/yk/Samsung_T5/Widar3.0ReleaseData/CSI/20181211.zip"
            "/media/yk/D4FA828F299D817A/Widar3.0ReleaseData/CSI/20181109.zip"
    ) as zip_f:
        for x in tqdm(zip_f.infolist()):
            if not x.is_dir():
                # if i < 5487:
                # continue
                # if i > 30:
                #     break
                b_all_data = zip_f.read(x)
                get_csi_from_bytes(b_all_data)
